...
export class MarketplaceService {
  ...
  async createListing(data: Omit<MarketplaceListingInsert, 'id' | 'created_at' | 'updated_at'>): Promise<{ listing: MarketplaceListing | null; error?: string }> {
    try {
  ...
  async updateListing(id: string, updates: Partial<MarketplaceListingInsert>): Promise<{ error?: string }> {
    try {
  ...
  async uploadListingImage(file: File, listingId: string): Promise<{ url: string; error?: string }> {
  ...
  async contactSeller(listingId: string, buyerId: string, message: string): Promise<{ error?: string }> {
    ...
    try {
      ...
      const { data: existing } = await supabase
        .from('conversations')
        .select('id')
        .or(`and(participant1_id.eq.${buyerId},participant2_id.eq.${listing.seller_id}),and(participant1_id.eq.${listing.seller_id},participant2_id.eq.${buyerId})`)
        .single();
