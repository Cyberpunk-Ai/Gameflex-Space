...
export class MessagesService {
  ...
  async sendMessage(conversationId: string, senderId: string, content: string): Promise<{ message: Message | null; error?: string }> {
    try {
      const { data, error } = await supabase
        .from('messages')
  ...
  async createConversation(participant1Id: string, participant2Id: string): Promise<{ conversation: Conversation | null; error?: string }> {
    try {
