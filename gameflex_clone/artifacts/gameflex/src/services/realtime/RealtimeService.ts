...
export class RealtimeService {
  ...
  subscribeToLeaderboard(callback: (payload: any) => void): () => void {
    ...
    const channel = supabase
      .channel(channelName)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'leaderboard_stats' }, callback)
      .subscribe();
  ...
  subscribeToPresence(channelName: string, userId: string, callback: (users: any[]) => void): { track: (state: any) => Promise<any>, unsubscribe: () => void } {
    ...
    return {
      track: (state: any) => channel.track({ user_id: userId, online_at: new Date().toISOString(), ...state }),
      unsubscribe: () => {
        supabase.removeChannel(channel);
  ...
  subscribeToFriendActivity(userId: string, callback: (payload: any) => void): () => void {
    ...
    const channel = supabase
      .channel(channelName)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'activity_feed', filter: `user_id=eq.${userId}` }, callback)
      .subscribe();
  ...
  subscribeToMatchUpdates(matchId: string, callback: (payload: any) => void): () => void {
    ...
    const channel = supabase
      .channel(channelName)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'matches', filter: `id=eq.${matchId}` }, callback)
      .subscribe();
