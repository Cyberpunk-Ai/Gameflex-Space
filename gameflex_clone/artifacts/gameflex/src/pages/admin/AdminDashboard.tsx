...
import { Users, Trophy, DollarSign, Clock, TrendingUp, Activity, Gamepad2, Bell, RefreshCw, Database } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const { data: health = { db: true, auth: true, payments: true } } = useQuery({
    ...
    queryFn: async () => {
      let db = false, auth = false, payments = false;
      try {
  ...
  const { data: stats } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [
        ...
        supabase.from('user_statuses').select('*', { count: 'exact', head: true }).gte('created_at', todayStart),
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {statusPills.map(pill => (
          <div key={pill.label} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-secondary/60 text-xs font-medium">
