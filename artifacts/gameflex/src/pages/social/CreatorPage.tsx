...
export default function CreatorPage() {
  ...
  const { data } = useQuery({
    queryKey: ['creator', id],
    queryFn: async () => {
      const [{ data: profile }, { data: posts }, { data: followers }, { data: likes }] = await Promise.all([
        supabase.from('profiles').select('*').eq('user_id', id).maybeSingle(),
        supabase.from('user_statuses').select('*').eq('user_id', id).order('created_at', { ascending: false }).limit(12),
        supabase.from('user_follows').select('follower_id', { count: 'exact', head: true }).eq('following_id', id),
    ...
    <SocialLayout>
      <div className="rounded-xl bg-gradient-to-br from-primary/20 to-primary/5 border border-primary/30 p-6 mb-6">
        <div className="flex items-center gap-4 mb-4">
          <Avatar className="h-20 w-20 border-2 border-primary"><AvatarImage src={profile.avatar_url} /><AvatarFallback>{profile.username?.[0]?.toUpperCase()}</AvatarFallback></Avatar>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
        {posts.map((p: any) => (
          <Link key={p.id} to={`/post/${p.id}`} className="aspect-square rounded-lg overflow-hidden bg-secondary block">
            {p.media_url ? (
              p.media_type === 'video'
                ? <video src={p.media_url} className="w-full h-full object-cover" muted />
                : <img loading="lazy" decoding="async" src={p.media_url} className="w-full h-full object-cover" />
...
function Stat({ icon: Icon, label, value }: any) {
  return (
    <div className="p-3 rounded-lg bg-background/50">
      ...
      <div className="font-display font-bold">{typeof value === 'number' ? value.toLocaleString() : value}</div>
