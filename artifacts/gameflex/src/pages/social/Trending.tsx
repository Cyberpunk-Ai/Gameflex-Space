  ...
  const { data: tags = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      data?.forEach((s: any) => {
        ...
        matches.forEach((t: string) => { counts[t.toLowerCase()] = (counts[t.toLowerCase()] ?? 0) + 1; });
      });
  ...
  return (
    <SocialLayout title="Trending" subtitle="Hot posts and hashtags this week">
      <div className="rounded-xl border border-border/50 bg-card p-4 mb-6">
        <h2 className="font-display font-bold text-sm mb-3 flex items-center gap-2"><Hash className="h-4 w-4" />Trending hashtags</h2>
        {tags.length === 0 ? (
          <p className="text-xs text-muted-foreground">Add #hashtags to your posts to see them trend.</p>
        ) : (
      ...
      <h2 className="font-display font-bold flex items-center gap-2 mb-3"><TrendingUp className="h-4 w-4 text-primary" />Top posts</h2>
