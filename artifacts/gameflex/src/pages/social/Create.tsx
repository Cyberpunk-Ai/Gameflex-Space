...
import { Upload, X, Globe, Lock, Loader2, Check, ImageIcon, Video } from 'lucide-react';
...
const GRADIENTS = [
  { id: 'neon',     label: 'Neon',     css: 'linear-gradient(135deg, hsl(142 76% 45%) 0%, hsl(180 100% 50%) 100%)' },
  { id: 'victory',  label: 'Victory',  css: 'linear-gradient(135deg, hsl(45 100% 50%) 0%, hsl(142 76% 45%) 100%)' },
  { id: 'forest',   label: 'Forest',   css: 'linear-gradient(135deg, hsl(142 76% 28%) 0%, hsl(160 80% 45%) 100%)' },
  { id: 'midnight', label: 'Midnight', css: 'linear-gradient(135deg, hsl(220 80% 40%) 0%, hsl(142 76% 45%) 100%)' },
  { id: 'ember',    label: 'Ember',    css: 'linear-gradient(135deg, hsl(25 100% 55%) 0%, hsl(142 76% 45%) 100%)' },
  { id: 'aurora',   label: 'Aurora',   css: 'linear-gradient(135deg, hsl(280 80% 55%) 0%, hsl(142 76% 45%) 100%)' },
];
...
export default function Create() {
  ...
  const handleFile = (f?: File | null) => {
    ...
    if (!f.type.startsWith('image/') && !f.type.startsWith('video/')) {
      toast({ title: 'Images and videos only', variant: 'destructive' }); return;
    }
    setFile(f);
        ...
        <div className="mb-4">
          ...
          {filePreview ? (
            ...
            <div className="relative rounded-2xl overflow-hidden bg-black shadow-md aspect-video">
              ...
              <button onClick={clearFile} className="absolute top-3 right-3 h-8 w-8 rounded-full bg-black/60 backdrop-blur-md text-white flex items-center justify-center hover:bg-black/80 transition-colors">
                <X className="h-4 w-4" />
              </button>
            </div>
          ) : textMode ? (
              ...
              {GRADIENTS.map(g => (
                <button
                  ...
                  className={cn(
                    ...
                    gradient === g.css ? 'ring-2 ring-primary ring-offset-1 ring-offset-background' : ''
                  )}
            ...
            {[
              ...
              <button
                ...
                className={cn(
                  ...
                  visibility === id ? 'bg-primary/10 border-primary text-primary' : 'bg-secondary/30 border-border/50 hover:bg-secondary/50'
                )}
              >
