...
export type StatusItem = Database['public']['Tables']['user_statuses']['Row'] & { profiles?: Profile; _count?: { likes: number; comments: number }; liked?: boolean };
...
export class SocialService {
  ...
  async createStatus(userId: string, content: string, imageUrl?: string): Promise<{ status: StatusItem | null; error?: Error }> {
    try {
      const { data, error } = await supabase
        .from('user_statuses')
  ...
  async likeStatus(userId: string, statusId: string): Promise<{ error?: Error }> {
    try {
      const { error } = await supabase.from('status_likes').insert({ user_id: userId, status_id: statusId });
      if (error) throw error;
      return {};
    } catch (err: any) {
      return { error: err instanceof Error ? err : new Error(String(err)) };
  ...
  async unlikeStatus(userId: string, statusId: string): Promise<{ error?: Error }> {
    try {
      const { error } = await supabase.from('status_likes').delete().eq('user_id', userId).eq('status_id', statusId);
      if (error) throw error;
      return {};
    } catch (err: any) {
      return { error: err instanceof Error ? err : new Error(String(err)) };
  ...
  async commentOnStatus(userId: string, statusId: string, content: string): Promise<{ comment: any; error?: Error }> {
    try {
      const { data, error } = await supabase
