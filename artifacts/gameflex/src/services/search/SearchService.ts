...
export type SearchableEntity = 'players' | 'teams' | 'creators' | 'posts' | 'tournaments' | 'products' | 'communities';
...
export interface ISearchProvider {
  search(query: string, options: { entities?: SearchableEntity[]; limit?: number }): Promise<SearchResult[]>;
}

export class SupabaseSearchProvider implements ISearchProvider {
  async search(query: string, options: { entities?: SearchableEntity[]; limit?: number }): Promise<SearchResult[]> {
...
export class SearchService {
  ...
  async search(query: string, entities?: SearchableEntity[], limit?: number): Promise<SearchResult[]> {
