...
import {
  Home, Search, Compass, Film, MessageCircle, PlusSquare,
  Menu, Trophy, Bookmark, Activity as ActivityIcon, Users, Shield,
  Radio, TrendingUp, Settings, LogOut, User as UserIcon, LayoutDashboard,
  Circle, X, Bell, ChevronRight, Camera
} from 'lucide-react';
...
const moreItems = [
  ...
  { name: 'Settings', href: '/social/settings', icon: Settings },
...
function MoreMenu({ collapsed }: { collapsed: boolean }) {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent side="top" align={collapsed ? "center" : "start"} className="w-64 bg-card border-border/50 p-2 shadow-2xl mb-2 rounded-2xl">
        {user && (
          <>
        ...
        {moreItems.map(it => (
          <DropdownMenuItem key={it.name} asChild className="rounded-xl cursor-pointer">
            <Link to={it.href} className={cn("w-full py-2.5", isActive(it.href) && "bg-secondary/80 font-semibold")}>
        ...
        {user && (
          <DropdownMenuItem onClick={logout} className="rounded-xl cursor-pointer text-destructive focus:bg-destructive/10 focus:text-destructive py-2.5">
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex items-center gap-4">
        <Icon className={cn("h-6 w-6 shrink-0 transition-transform group-hover:scale-110", active && "stroke-[2.5] text-primary")} />
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-1">
        <div className="hidden xl:block px-2"><BrandLink /></div>
        <div className="xl:hidden flex justify-center"><BrandLink collapsed /></div>

        <div className="flex flex-col gap-1 mt-2">
          {primary.map((it) => (
            <div key={it.name} className="xl:block">
              <div className="xl:hidden"><SidebarItem item={it} collapsed /></div>
              <div className="hidden xl:block"><SidebarItem item={it} collapsed={false} /></div>
            </div>
          ))}
            ...
            >
              ...
              <div className="relative z-10 flex items-center gap-4">
                <div className="relative">
                  <Avatar className={cn("h-7 w-7 transition-transform group-hover:scale-110", activeProfile && "ring-2 ring-primary ring-offset-2 ring-offset-background")}>
      ...
      <div className="flex flex-col gap-1 mt-auto px-2">
        <div className="xl:hidden"><MoreMenu collapsed /></div>
        <div className="hidden xl:block"><MoreMenu collapsed={false} /></div>
      </div>
    </motion.aside>
  );
}
  ...
  return (
    <header className="md:hidden sticky top-0 z-40 flex items-center justify-between px-4 h-14 border-b border-border/30 bg-background/80 backdrop-blur-xl">
      ...
      <div className="flex items-center gap-2">
        <Link to="/explore" className="p-2 rounded-full hover:bg-secondary/60 active:scale-95 transition-transform">
          <Compass className="h-6 w-6" />
        </Link>
        <Link to="/notifications" className="p-2 rounded-full hover:bg-secondary/60 active:scale-95 transition-transform">
          <Bell className="h-6 w-6" />
        </Link>
        <Link to="/messages" className="p-2 rounded-full hover:bg-secondary/60 active:scale-95 transition-transform">
...
function MobileBottomNav() {
  ...
  const sheetItems = [
    ...
    { name: 'Teams', href: '/teams', icon: Shield, desc: 'Team hub' },
    { name: 'Live', href: '/live', icon: Radio, desc: 'Live streams' },
    { name: 'Leaderboard', href: '/leaderboard', icon: Trophy, desc: 'Top players' },
  ];

  if (isAdmin) {
    sheetItems.push({ name: 'Dashboard', href: '/dashboard', icon: LayoutDashboard, desc: 'Admin panel' });
  }
    ...
    <>
      <nav className="md:hidden fixed bottom-0 inset-x-0 z-40 border-t border-border/30 bg-background/95 backdrop-blur-2xl h-[64px] pb-safe flex items-center justify-around px-1 shadow-[0_-8px_30px_rgba(0,0,0,0.18)]">
        {primaryItems.map(it => {
          ...
          if (isCreate) {
            return (
              <Link key={it.name} to={it.href} className="flex flex-col items-center justify-center w-full h-full p-2 active:scale-90 transition-transform">
          ...
          return (
            <Link key={it.name} to={it.href} className="flex flex-col items-center justify-center w-full h-full p-2 active:scale-90 transition-transform relative">
              <it.icon className={cn("h-[26px] w-[26px] transition-colors duration-150", active ? "stroke-[2.5] text-foreground" : "text-muted-foreground")} />
              {active && <span className="absolute bottom-1.5 w-1 h-1 rounded-full bg-primary shadow-[0_0_4px_rgba(34,197,94,0.9)]" />}
            </Link>
          );
        })}
        ...
        <Sheet open={moreOpen} onOpenChange={setMoreOpen}>
          ...
          <SheetContent side="bottom" className="rounded-t-3xl max-h-[85dvh] pb-safe bg-card border-border/50 p-0">
            <div className="px-4 pt-4 pb-2">
              ...
              {user && (
                <SheetClose asChild>
                  <Link to="/social/profile" className="flex items-center gap-3 py-3 px-2 rounded-2xl hover:bg-secondary/50 active:bg-secondary/70 transition-colors mb-2">
            ...
            <div className="overflow-y-auto px-4 pb-6">
                ...
                {sheetItems.map(it => {
                  ...
                  return (
                    <SheetClose asChild key={it.name}>
                      <Link to={it.href} className={cn(
                        "flex flex-col items-center gap-1.5 py-3 px-2 rounded-2xl transition-all active:scale-95",
                        active ? "bg-primary/15 border border-primary/30" : "bg-secondary/30 hover:bg-secondary/50"
                      )}>
                        <div className={cn(
                        ...
                        <span className={cn("text-[11px] font-semibold truncate w-full text-center leading-tight", active ? "text-primary" : "text-foreground/80")}>
                          {it.name}
                        </span>
                      </Link>
                    </SheetClose>
              ...
              <div className="border-t border-border/30 pt-3 flex gap-2">
                <SheetClose asChild>
                  <Link to="/social/settings" className="flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl bg-secondary/30 hover:bg-secondary/50 transition-colors text-sm font-medium text-muted-foreground active:scale-95">
...
export function SocialNav() { return null; }