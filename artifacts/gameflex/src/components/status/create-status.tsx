          ...
          {mediaPreview && (
            <div className="relative mt-3 rounded-xl overflow-hidden border border-border/50">
              {mediaFile?.type.startsWith('video') ? (
                <video src={mediaPreview} className="w-full max-h-48 object-cover" muted controls={false} />
              ) : (
            ...
            {isExpanded && (
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  <span className={cn("text-xs", content.length > 250 ? "text-destructive" : "text-muted-foreground")}>
                    {content.length}/280
                  </span>
                  <Button
                    size="sm"
                    className="h-8 px-4 rounded-full font-bold bg-primary text-primary-foreground hover:bg-primary/90"
                    disabled={(!content.trim() && !mediaFile) || content.length > 280 || isSubmitting}
