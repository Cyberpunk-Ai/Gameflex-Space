...
export function ConversationList({ selectedConversationId, onSelectConversation, onCompose }: ConversationListProps) {
  const { user } = useAuth();
  ...
  const { data: conversations = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const result = convData.map(c => {
        ...
        const isOnline = lastMsg ? differenceInMinutes(new Date(), new Date(lastMsg.created_at)) < 10 : false;
        return {
          ...c,
  ...
  const earlierConvs = filtered.filter(c => !c.lastMsg || !isToday(new Date(c.lastMsg.created_at)));
  ...
  function ConversationItem({ conversation }: { conversation: any }) {
    ...
    return (
      <button
        ...
        style={isSelected ? {
          background: 'linear-gradient(135deg, hsl(142 76% 45% / 0.14) 0%, hsl(160 80% 45% / 0.07) 100%)',
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2">
            <span className={cn("font-medium truncate text-sm", hasUnread && "font-bold", isSelected && "text-primary")}>
          ...
          <div className="flex items-center justify-between gap-2 mt-0.5">
            <span className={cn("text-xs text-muted-foreground truncate", hasUnread && "font-semibold text-foreground")}>
              {preview}
            </span>
            {hasUnread && (
          ...
          <>
            {todayConvs.length > 0 && (
              <>
                ...
                {todayConvs.map(c => <ConversationItem key={c.id} conversation={c} />)}
              </>
            )}
            {earlierConvs.length > 0 && (
              <>
                <GroupLabel label="Earlier" />
                {earlierConvs.map(c => <ConversationItem key={c.id} conversation={c} />)}
              </>
            )}
          </>
        )}
