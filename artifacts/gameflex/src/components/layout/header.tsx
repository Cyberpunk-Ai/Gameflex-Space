...
export function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 backdrop-blur-md bg-background/80 border-b border-border/50" role="banner">
      <nav className="container mx-auto flex items-center justify-between p-4 min-h-[72px]" role="navigation" aria-label="Main navigation">
        ...
        <div className="hidden lg:flex items-center gap-3">
          {isAuthenticated && user ? (
            <>
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-56 bg-card border-border/50">
                  ...
                  <DropdownMenuItem asChild>
                    <Link to="/social/profile" className="cursor-pointer"><User className="mr-2 h-4 w-4" />Profile</Link>
                  </DropdownMenuItem>
                  <DropdownMenuItem asChild>
                    <Link to="/social/settings" className="cursor-pointer"><Settings className="mr-2 h-4 w-4" />Settings</Link>
                  </DropdownMenuItem>
                  {isAdmin && (
                    <>
                      <DropdownMenuSeparator />
            ...
            <>
              ...
              <Button variant="neon" asChild><Link to="/register">Join Now</Link></Button>
            </>
          )}
        </div>
            ...
            <div className="pt-4 border-t border-border/50 space-y-2">
              {isAuthenticated && user ? (
                <>
                  ...
                  <Link to="/dashboard" onClick={() => setMobileMenuOpen(false)} className="block px-4 py-3 rounded-lg text-sm font-medium hover:bg-secondary min-h-[44px] flex items-center">Dashboard</Link>
                  <Link to="/social/profile" onClick={() => setMobileMenuOpen(false)} className="block px-4 py-3 rounded-lg text-sm font-medium hover:bg-secondary min-h-[44px] flex items-center">Profile</Link>
                  {isAdmin && (
                    <Link to="/admin" onClick={() => setMobileMenuOpen(false)} className="block px-4 py-3 rounded-lg text-sm font-medium hover:bg-secondary min-h-[44px] flex items-center">Admin Panel</Link>
                  )}
                  <button onClick={() => { logout(); setMobileMenuOpen(false); }} className="w-full text-left px-4 py-3 rounded-lg text-sm font-medium text-destructive hover:bg-destructive/10 min-h-[44px] flex items-center">Logout</button>
                </>
              ) : (
                <>
                  <Link to="/login" onClick={() => setMobileMenuOpen(false)} className="block px-4 py-3 rounded-lg text-sm font-medium hover:bg-secondary min-h-[44px] flex items-center">Sign In</Link>
                  <Link to="/register" onClick={() => setMobileMenuOpen(false)} className="block px-4 py-3 rounded-lg text-sm font-medium bg-primary text-primary-foreground text-center min-h-[44px] flex items-center justify-center">Join Now</Link>
                </>
              )}
            </div>
          </div>
