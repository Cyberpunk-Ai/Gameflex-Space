...
export function useCreateConversation() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ participant1Id, participant2Id }: { participant1Id: string; participant2Id: string }) =>
      messagesService.createConversation(participant1Id, participant2Id),
    onSuccess: (_, vars) => {
      queryClient.invalidateQueries({ queryKey: QUERY_KEYS.messages.conversations(vars.participant1Id) });
    },
  });
}
