...
export function useJoinTournament() {
  ...
  return useMutation({
    mutationFn: ({ tournamentId, userId, gameHandle, paymentId }: {
      tournamentId: string; userId: string; gameHandle: string; paymentId?: string;
    }) => tournamentService.joinTournament(tournamentId, userId, gameHandle, paymentId),
    onSuccess: (_, vars) => {
      queryClient.invalidateQueries({ queryKey: QUERY_KEYS.tournaments.registrations(vars.tournamentId) });
    },
  });
}
